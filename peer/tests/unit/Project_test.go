...
// 		2. 		2 - 1 ACK
// 		3. 		2 - 1 MulticastJoinMessage (contains StreamJoinMessage)
...
//		1. 		1 - 2 Rumor (with StartStreamMessage)
//		2. 		2 - 1 ACK
//		3.		2 - 3 Rumor (with StartStreamMessage)
// 		4. 		3 - 2 ACK

// 		Node 3 wants to join the stream
// 		5. 		3 - 2 MulticastJoinMessage (contains StreamJoinMessage)
// 		6. 		2 - 1 MulticastJoinMessage (contains StreamJoinMessage)
//		7.		1 - 3 StreamAcceptMessage

//		Node 1 streams:
// 		8. 		1 - 2 MulticastMessage(StreamDataMessage(StreamMessage))
// 		9. 		2 - 3 MulticastMessage(StreamDataMessage(StreamMessage))
...
func Test_Project_RelayedStream(t *testing.T) {
	...
	n3Ins := node3.GetIns()
	require.Len(t, n3Ins, 5)

	require.Equal(t, "rumor", n3Ins[0].Msg.Type)
	rumors = z.GetRumor(t, n3Ins[0].Msg)
	require.Len(t, rumors.Rumors, 1)
	require.Equal(t, "streamstartmessage", rumors.Rumors[0].Msg.Type)
	require.Equal(t, "streamacceptmessage", n3Ins[1].Msg.Type)

	require.Equal(t, "multicastmessage", n3Ins[2].Msg.Type)
	multicastMsg = z.GetMulticastMessage(t, n3Ins[2].Msg)
	require.Equal(t, "streamdatamessage", multicastMsg.Message.Type)

	require.Equal(t, "multicastmessage", n3Ins[3].Msg.Type)
	multicastMsg = z.GetMulticastMessage(t, n3Ins[3].Msg)
	require.Equal(t, "streamdatamessage", multicastMsg.Message.Type)

	require.Equal(t, "multicastmessage", n3Ins[4].Msg.Type)
	multicastMsg = z.GetMulticastMessage(t, n3Ins[4].Msg)
	require.Equal(t, "streamdatamessage", multicastMsg.Message.Type)
	...
	require.Len(t, n3Outs, 2)
	require.Equal(t, "ack", n3Outs[0].Msg.Type)
	require.Equal(t, "multicastjoinmessage", n3Outs[1].Msg.Type)
	multicastJoinMsg = z.GetMulticastJoinMessage(t, n3Outs[1].Msg)
	require.Equal(t, "streamconnectmessage", multicastJoinMsg.Message.Type)
	log.Info().Msgf("Test Done")
